\usepackage{graphicx} % Required for inserting images
\usepackage{wrapfig,lipsum}
\usepackage[utf8]{inputenc} %lettere accentate da tastiera 
\usepackage[T1]{fontenc} % higher quality font encoding
%load the font and set it to default
\usepackage{amsmath,amsthm,amsfonts,amssymb}
\DeclareMathOperator*{\argmin}{argmin}
\DeclareMathOperator*{\argmax}{argmax} % Già che ci sei, definisci anche questo
\numberwithin{equation}{section}
\usepackage[english]{babel}
\usepackage{url}
\usepackage{frontespizio}
\usepackage{geometry}
\geometry{a4paper,top=3cm,bottom=3cm,left=3.5cm,right=3.5cm,%
	heightrounded,bindingoffset=5mm}
\usepackage[x11names]{xcolor}
\usepackage[most]{tcolorbox}
\tcbuselibrary{theorems}
\usepackage{physics}
\usepackage{hyperref}
\hypersetup{
	colorlinks=true,
	linkcolor=blue,
	filecolor=magenta,      
	urlcolor=blue,
	pdftitle={Tesi},
	pdfpagemode=FullScreen,
}
\usepackage{enumitem}
\usepackage[toc,page]{appendix}
\usepackage[backend=biber , style = numeric]{biblatex}
\usepackage{pst-eucl,pst-plot}



\newtheorem{teorema}{Teorema}[subsection]

\theoremstyle{definition}
\newtheorem{definizione}{Definition}[section]

\newtheorem*{proprieta}{Proprietà}
\newtheorem{corollario}{Corollario}[teorema]
\newtheorem*{formula}{Formula}
\newtheorem{prop}{Proposizione}[section]
\newtheorem{es}{Esempio}[section]
\newtheorem{lemma}{Lemma}[section]
%\newtcbtheorem[number within=section]{teo}{Theorem}{colback=LightCyan1!40 ,colframe=RoyalBlue1!100,sharp corners,separator sign dash,fonttitle=\bfseries}{thm}
\newtcbtheorem[number within=section]{teo}{Theorem}{colback=white ,colframe=black,sharp corners,separator sign dash,fonttitle=\bfseries}{thm}
\newtcbtheorem[number within=section]{teo1}{}{colback=black!5 ,colframe=Burlywood4!80 }{thm}


\newcommand{\A}{\mathcal{A}}
\newcommand{\R}{\mathbb{R}}
\newcommand{\D}{\mathbb{D}}
\newcommand{\V}{\mathbb{V}}
\newcommand{\K}{\mathbb{K}}
\newcommand{\w}{\mathbb{W}}
\newcommand{\C}{\mathbb{C}}
\newcommand{\la}{\lambda}
\newcommand{\on}{^{\perp}}
\newcommand{\xb}{\overline{x}}
\newcommand{\te}{\theta}
\newcommand{\fn}{f: A\subseteq \Rn \rightarrow \R}
\newcommand{\fnn}{f: A\subseteq \Rn \rightarrow \Rn}
\newcommand{\fnm}{f: A\subseteq \Rn \rightarrow \R^m}
\renewcommand{\labelitemi}{$\star$}
\renewcommand{\arraystretch}{1.2} % Aumenta l'altezza delle righe
