...
\end{align*}
We want to prove that there exist a crtical anglie $\te_c \in ]-\pi/2 , \pi/2[$ . We note that $g(\te) > 0 \forall \te \in ] -\pi/2 , pi/2 [$ , so if we want the accelator the have the opposite sign of $\te$ we nedd to garantee that $h(\te , \dot{\te} , u) \geq 0 $
\begin{align*}
...
\end{align*} 
 ...
 \begin{itemize}
 \item $\sin(\te) > 0 $ -> $\tan(\te) \geq -\frac{u_{min}}{g(M+m)}$
 \item $\sin(\te)<0$ -> $\tan(\te) \leq -\frac{u_{max}}{g(M+m)} $
 \end{itemize}
 so we obtain 
 \[\atan(-\frac{u_{max}}{g(M+m)} )\leq \theta \leq \atan(-\frac{u_{min}}{g(M+m)} )\]
